...
SAMPLES, = glob_wildcards(f"{INPUT_DIR}/{{sample}}_S*_L006_R1_001.fastq.gz")
...
rule fastqc:
    input:
        r1=f"{INPUT_DIR}/{{sample}}_S*_L006_R1_001.fastq.gz",
        r2=f"{INPUT_DIR}/{{sample}}_S*_L006_R2_001.fastq.gz"
    output:
...
rule summarize_human_read_loss:
    input:
        ...
        expand(f"{INPUT_DIR}/{{sample}}_S*_L006_R1_001.fastq.gz", sample=SAMPLES),
        expand(f"{INPUT_DIR}/{{sample}}_S*_L006_R2_001.fastq.gz", sample=SAMPLES)
    output:
        "step1_bowtie2/human_read_loss_summary.txt"
    params:
    ...
    run:
        ...
        with open(output[0], "w") as out:
            ...
            for sample in SAMPLES:
                # Define file paths
                r1_in = f"{params.input_dir}/{sample}_R1.fastq.gz"
                r2_in = f"{params.input_dir}/{sample}_R2.fastq.gz"
