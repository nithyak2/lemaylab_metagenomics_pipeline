...
# to run this.. assuming Snakefile is in a separate folder than the running directory
## cd /path/to/project_root
# load snakemake into a conda environment if necessary
## eval "$(mamba shell hook --shell bash)"
## conda activate snakemake_env
# to run snakemake v8 onwards with slurm run the following (only needs to be done once)
## pip install snakemake-executor-plugin-slurm
# quick check to make sure there are no errors
## snakemake -s scripts/Snakefile -n
# to run actual script, submit an sbatch file with the following
...
# Auto-detect samples 
# this will need to be modified based on your project 
# if your raw files are not in a "fastq_files" folder, you will also need to modify the fastq_files path as needed in step 0 and step 1 rules
SAMPLES, = glob_wildcards("fastq_files/{sample}S*_R1_001.fastq.gz")

# set default resources, needed for slurm scheduler
GB = 1024
...
rule all:
    input:
        ...
        "step3_metaphlan/fl119_regularseq_NOVA1283P_sgb.metaphlan4-2-2.txt",
        "step3_metaphlan/fl119_regularseq_NOVA1283P_gtdb.metaphlan4-2-2.txt"
...
rule fastqc:
    input:
        r1="fastq_files/{sample}_R1.fastq.gz",
        r2="fastq_files/{sample}_R2.fastq.gz"
    output:
...
rule summarize_human_read_loss:
    input:
        ...
        # Original input files - ADD THESE
        expand("fastq_files/{sample}_R1.fastq.gz", sample=SAMPLES),
        expand("fastq_files/{sample}_R2.fastq.gz", sample=SAMPLES)
    output:
        "step1_bowtie2/human_read_loss_summary.txt"
    params:
    ...
    run:
        ...
        with open(output[0], "w") as out:
            ...
            for sample in SAMPLES:
                # Define file paths
                r1_in = f"fastq_files/{sample}_R1.fastq.gz"
                r2_in = f"fastq_files/{sample}_R2.fastq.gz"
...
# https://github.com/biobakery/MetaPhlAn/wiki/MetaPhlAn-4.2
########################################
rule gtdb:
    input: 
        profile="step3_metaphlan/sgb_profile/{sample}.sgb.metaphlan4-2-2.txt"
...
rule combine_metaphlan:
    input:
        profiles=expand("step3_metaphlan/sgb_profile/{sample}.sgb.metaphlan4-2-2.txt", sample=SAMPLES),
        profiles2=expand("step3_metaphlan/gtdb_profile/{sample}.gtdb.metaphlan4-2-2.txt", sample=SAMPLES)
    output:
        merged="step3_metaphlan/fl119_regularseq_NOVA1283P_sgb.metaphlan4-2-2.txt",
        merged2="step3_metaphlan/fl119_regularseq_NOVA1283P_gtdb.metaphlan4-2-2.txt"
    conda:
        "/quobyte/dglemaygrp/nkumar/conda_envs/metaphlan.yaml"
    shell:
        """
        scripts/merge_metaphlan_tables.py {input.profiles} > {output.merged}
        scripts/merge_metaphlan_tables.py {input.profiles2} > {output.merged2}
